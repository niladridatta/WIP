...
for s in strike:
	...
	for opt in opt_type:
		...
		if opt == 'CE':
			c.execute("SELECT SYMBOL, INSTRUMENT, EXPIRY_DT, STRIKE_PR, OPTION_TYP, CLOSE, TIMESTAMP FROM Oct_2017 WHERE EXPIRY_DT = ? AND STRIKE_PR = ? AND OPTION_TYP = ? ORDER BY TIMESTAMP", (expiry, s, opt))
